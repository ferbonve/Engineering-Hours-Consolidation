...
![Input file structure](assets/user.png)
...
The consolidated `.xlsx` output generated by this pipeline was used as the data source for a **Power BI dashboard**, enabling real time visualization of engineering hour allocation across clients, areas, and business units.
 
> Python handles the ETL complexity (multi-file ingestion, wide→long reshape, holiday logic) — Power BI handles the visualization. Clean separation of concerns.
 
![Power BI Report - Abril 2024](assets/reporte_powerbi.png)
 
### What the dashboard shows
 
| Visual | Description |
|---|---|
| KPI cards | Total hours, active clients, top client, main area |
| Horizontal bar chart | % of hours per client — ranked top to bottom |
| Donut — Por Área | Distribution across Excelencia Operativa, Pricing, Planeamiento |
| Donut — Por UUNN | Distribution across business units (UNL, UNT, UNP) |
| Line chart | Weekly accumulated hours by area throughout the month |
 
### Stack
 
```
Individual .xlsm files  →  Python ETL pipeline  →  Consolidado Horas.xlsx  →  Power BI
```
